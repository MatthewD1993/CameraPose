...
parser.add_argument("--data_dir",type=str, default="tmp/logs/orientationNetwork_with_summaries", help="Summaries log directory")
...
if tf.gfile.Exists(arguments.data_dir):
    tf.gfile.DeleteRecursively(arguments.data_dir)
tf.gfile.MakeDirs(arguments.data_dir)
...
train_writer = tf.summary.FileWriter(arguments.data_dir + '/train', sess.graph)
test_writer = tf.summary.FileWriter(arguments.data_dir + '/test')
...
for epoch in range(epochs+1):
    ...
    summary = sess.run(merged, {network_input: input_data, estimate: ground_truth})
